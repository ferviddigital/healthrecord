...
<template>
  <form @submit.prevent="emit('submit', person)">
    ...
    <span class="block text-gray-500 text-sm font-light mb-1">
      Sex
    </span>
    <input class="peer/male" v-model="sex" type="radio" name="sex" id="male" value="Male" required>
    <label for="male" class="inline align-text-bottom text-sm ml-1 peer-checked/male:text-indigo-500 peer-checked/male:font-normal"> Male</label>
    <input class="peer/female ml-4" v-model="sex" type="radio" name="sex" id="female" value="Female" required>
    <label for="female" class="inline align-text-bottom ml-1 peer-checked/female:text-indigo-500 peer-checked/female:font-normal"> Female</label>
    <label class="mt-3" for="dob">
      Date of Birth
    </label>
